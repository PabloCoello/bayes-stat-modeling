...
The `templates` folder contains base templates built following good practices to follow at the time of building statistical models.
...
The fields in the table should be filled with the following information:

* **Model id**: A short code or name for an specific modelization.
* **Description**: A short description of what was attempted in this specific modelization that is different from the others.
* **Result**: One of the following possible results:
  ...
  * __Model is provisionally accepted__: This model is provisionally accepted and is considered in the model comparison step.
...
4. **Verify the installation**
   ...
   - Test Quarto: `quarto --version`

## Running an Analysis

1. Navigate to a model folder (e.g., `models/eight-schools/`)
2. Open the `.qmd` file
3. Click "Render" in VS Code or run:
   ```bash
   quarto render eight-schools.qmd
   ```
